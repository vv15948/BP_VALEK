function collision = is_robot_model_in_collision(positions, voxel_grid, range,dir)
% Rychlejší kontrola kolizí pomocí válců + sféry na posledním kloubu

    % Poloměr válců a koule
    radius1 = 0.08;
    radius2 = 0.07;
    radius = 0.06;
    dir1 = [dir(2), -dir(1), dir(3)];
    % Kontrola každého válce zvlášť
    for i = 1:5
        if i==1
        start_point = positions(i,:)+dir1.*0.14;
        end_point = positions(i+1,:)+dir1.*0.14;
        a = end_point-start_point;
        dir2=a/norm(a);
        offset2 = (dir2*0.072);
        start_point = start_point - offset2;
        end_point = end_point + offset2;
        % Vytvoříme masku jen pro tento válec
        cylinder_mask = add_cylinder_to_voxel_grid(false(size(voxel_grid)), ...
            range(1,:), range(2,:), range(3,:), ...
            start_point, end_point, radius1);
        elseif i ==2
            a = positions(i+1,:)-positions(i,:);
        dir2=a/norm(a);
        offset2 = (dir2*0.06);
        start_point = positions(i,:)- offset2;
        end_point = positions(i+1,:)+offset2;
        % Vytvoříme masku jen pro tento válec
            cylinder_mask = add_cylinder_to_voxel_grid(false(size(voxel_grid)), ...
            range(1,:), range(2,:), range(3,:), ...
            start_point, end_point, radius2);
        elseif i==3
        a = positions(i+1,:)-positions(i,:);
        dir2=a/norm(a);
        offset2 = (dir2*0.03);
        start_point = positions(i,:)- offset2;
        end_point = positions(i+1,:)+offset2;
        % Vytvoříme masku jen pro tento válec
            cylinder_mask = add_cylinder_to_voxel_grid(false(size(voxel_grid)), ...
            range(1,:), range(2,:), range(3,:), ...
            start_point, end_point, radius);
        else
        start_point = positions(i,:);
        end_point = positions(i+1,:);
        cylinder_mask = add_cylinder_to_voxel_grid(false(size(voxel_grid)), ...
            range(1,:), range(2,:), range(3,:), ...
            start_point, end_point, radius);
        end



        % Pokud došlo ke kolizi – ihned skončíme
        if any(cylinder_mask(:) & voxel_grid(:))
            collision = true;
            return;
        end
    end


    % Pokud jsme prošli vše bez kolize:
    collision = false;
end
